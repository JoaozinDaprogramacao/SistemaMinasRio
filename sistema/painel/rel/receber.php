...
// Consulta base
$sql = "SELECT * FROM receber WHERE 1";

// Filtro por data e status de pagamento
if ($mostrar_registros == 'Não') {
    if ($pago == 'Vencidas') {
        $sql .= " AND vencimento < curDate() AND pago != 'Sim' AND usuario_lanc = '$id_usuario'";
    } else {
        $sql .= " AND $tipo_data >= '$dataInicial' AND $tipo_data <= '$dataFinal' AND pago LIKE '%$pago%' AND usuario_lanc = '$id_usuario'";
    }
} else {
    if ($pago == 'Vencidas') {
        $sql .= " AND vencimento < curDate() AND pago != 'Sim'";
    } else {
        $sql .= " AND $tipo_data >= '$dataInicial' AND $tipo_data <= '$dataFinal' AND pago LIKE '%$pago%'";
    }
}

// Filtro por atacadista
if (!empty($atacadista)) {
    $sql .= " AND cliente = '$atacadista'";
}
