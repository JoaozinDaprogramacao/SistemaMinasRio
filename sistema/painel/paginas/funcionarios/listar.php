...
HTML;
    ...
    for ($i = 0; $i < $linhas; $i++) {
        ...
        // Lógica de Ativação/Desativação baseada no novo campo 'status'
        ...
        // Escapando aspas para o javascript
...
<td>
    <big><a class="btn btn-info btn-sm" href="#" onclick="editar('{$id}', '{$nome_js}', '{$telefone}', '{$endereco_js}', '{$chave_pix}', '{$cargo_id}', '{$data_admissao}', '{$status}', '{$data_demissao}', '{$descricao_salario}', '{$obs_js}')" title="Editar Dados"><i class="fa fa-edit"></i></a></big>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#tabela').DataTable({
            "language": {
                // CDN para tradução, se necessário
...
<script type="text/javascript">
    function editar(id, nome, telefone, endereco, chave_pix, cargo_id, data_admissao, status, data_demissao, descricao_salario, obs) {
        ...
        // Preenchendo todos os campos do novo modal
        $('#id').val(id);
        $('#nome').val(nome);
        $('#telefone').val(telefone);
        $('#endereco').val(endereco);
        $('#chave_pix').val(chave_pix);
        $('#cargo').val(cargo_id).change(); // .change() é importante para o select
        ...
        // Funções para atualizar a interface do modal
        toggleDemissao(); // Garante que o campo de demissão apareça se necessário
        mascara_decimal_ponto(document.getElementById('descricao_salario')); // Formata o campo de descrição
        calcularSalarioFolha(); // Calcula o salário com base na descrição
    ...
    function mostrar(nome, telefone, endereco, chave_pix, cargo, data_admissao, status, data_demissao, salario_folha, obs, foto, data_cad) {
        // Para esta função, seu #modalDados precisa ser atualizado.
        // Veja a sugestão na próxima seção da resposta.
        $('#titulo_dados').text(nome);
        $('#telefone_dados').text(telefone);
        $('#endereco_dados').text(endereco);
        $('#pix_dados').text(chave_pix);
        $('#cargo_dados').text(cargo); // Novo
        $('#admissao_dados').text(data_admissao); // Novo
        $('#status_dados').text(status); // Novo
        $('#demissao_dados').text(data_demissao); // Novo
        $('#salario_dados').text(salario_folha); // Novo
        $('#obs_dados').text(obs); // Novo
    ...
    function limparCampos() {
        ...
        // Limpa a seleção de múltiplos
        $('#ids').val('');
        $('#btn-deletar').hide();
    }
    
    // Nova função para mudar o status de forma mais simples
    function mudarStatus(id, novo_status) {
        $.ajax({
            url: 'paginas/' + pag + "/mudar-status.php", // Este arquivo PHP precisa ser ajustado
            method: 'POST',
    ...
    // Funções de seleção e exclusão (mantidas como estavam)
