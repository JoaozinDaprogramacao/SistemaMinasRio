...
// SQL AJUSTADO: Agora busca contas onde pago é NULL ou diferente de 'Sim'
$query = $pdo->query("SELECT SUM(valor) as total FROM $tabela $query_base AND (pago IS NULL OR pago = '' OR pago = 'Não') AND vencimento < curDate()");
...
// Ajuste nos filtros da listagem também
if ($filtro == 'Vencidas') {
	$query = $pdo->query("SELECT * from $tabela where vencimento < curDate() and (pago IS NULL OR pago = '' OR pago = 'Não') $sql_usuario_lanc $sql_atacadista $sql_pgto $sql_funcionario order by id desc ");
	...
	$query = $pdo->query("SELECT * from $tabela where vencimento = '$data_amanha' and (pago IS NULL OR pago = '' OR pago = 'Não') $sql_usuario_lanc $sql_atacadista $sql_pgto $sql_funcionario order by id desc ");
	...
	$query = $pdo->query("SELECT * from $tabela WHERE $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' $sql_usuario_lanc $sql_atacadista $sql_pgto $sql_funcionario order by id desc ");
}
...
HTML;
	...
	for ($i = 0; $i < $linhas; $i++) {
		...
		// Se o banco vier NULL ou vazio, tratamos como 'Não' para a lógica visual
		if ($pago == "" || $pago == null) {
			$pago = "Não";
		}
		...
		$query2 = $pdo->query("SELECT nome FROM formas_pgto where id = '$forma_pgto'");
		$nome_pgto = $query2->fetchColumn() ?: 'Sem Registro';
...
<td>
    <big><a href="#" onclick="editar('{$id}','{$descricao}','{$valor}','{$fornecedor}','{$funcionario}','{$vencimento}','{$data_pgto}','{$forma_pgto}','{$frequencia}','{$obs}','{$arquivo}')" title="Editar Dados"><i class="fa fa-edit text-primary"></i></a></big>
    ...
    <big><a href="#" onclick="mostrar('{$descricao}','{$valor_finalF}','{$nome_pessoa}','{$vencimentoF}','{$data_pgtoF}','{$nome_pgto}','{$nome_frequencia}','{$obs}','{$arquivo}','','','','','','{$nome_usu_lanc}','{$nome_usu_pgto}', '{$pago}', '{$arquivo}', '', '', '{$tipo_pessoa}')" title="Mostrar Dados"><i class="fa fa-info-circle text-primary"></i></a></big>
