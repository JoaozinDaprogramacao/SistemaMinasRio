...
// Situação do desconto em relação ao que há para pagar no título.
...
function avisarDesconto() {
    ...
    if (est.excedente > 0) {
        // Desconto maior que o total: o fornecedor estaria devolvendo dinheiro.
        const aguardando = (descontoConfirmado !== est.assinatura);
        box.className = 'alert alert-danger py-2 px-3 mb-3';
        box.innerHTML =
            '<div class="fw-bold mb-1"><i class="fa fa-exclamation-triangle me-1"></i>Desconto maior que o valor a pagar</div>' +
            '<div class="small">O desconto de <b>' + moeda(est.desconto) + '</b> supera em <b>' + moeda(est.excedente) +
            '</b> o total do título (' + moeda(est.bruto) + '). Confirmando assim, o título é baixado zerado e a diferença some da despesa.</div>' +
        ...
        // Desconto exatamente igual ao total: legítimo (dívida perdoada), mas avisa.
        ...
        box.innerHTML =
            ...
            '</b> zera o título — nada será pago.</div>';
...
$(document).on('submit', '#form-baixar', function (e) {
    ...
    // Desconto acima do valor a pagar: segura o primeiro envio e só deixa passar
