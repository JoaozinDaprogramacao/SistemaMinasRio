...
document.getElementById('relatorio').addEventListener('click', function (event) {
    ...
    $.ajax({
        url: 'rel/pagar_class.php", // URL do script que processará a requisição',
        type: 'POST',
...
function definirPeriodo(periodo) {
    ...
    switch (periodo) {
        case 'hoje':
            dataInicio = dataFim = agora.toISOString().split('T')[0];
            break;
        case 'mes':
        ...
        case 'mes_passado':
            ...
            break;
        case 'ano':
            dataInicio = new Date(agora.getFullYear(), 0, 1).toISOString().split('T')[0];
            dataFim = new Date(agora.getFullYear(), 11, 31).toISOString().split('T')[0];
            break;
        default:
            return;
    }

    document.getElementById('dataInicial').value = dataInicio;
