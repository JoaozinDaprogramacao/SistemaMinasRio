...
// =================================================================================
// INÍCIO DO BLOCO "DEDO DURO" (TRY / CATCH / TRANSACTION)
// =================================================================================

try {
    $pdo->beginTransaction(); // Inicia transação segura

    // 1. TRATAMENTO DA DATA (Converte dd/mm/aaaa para aaaa-mm-dd se necessário)
    $data_mysql = $data;
    if (!empty($data) && strpos($data, '/') !== false) {
        // Assume formato PT-BR
        $timestamp = strtotime(str_replace('/', '-', $data));
        if ($timestamp) {
            $data_mysql = date('Y-m-d H:i:s', $timestamp);
        } else {
            // Fallback para data atual se a conversão falhar
    ...
    // 2. INSERT OU UPDATE DO CABEÇALHO
    $params = [
        ':forn'   => $fornecedor,
        ':cli'    => $cliente,
        ':qd'     => $quant_dias,
        ':dt'     => $data_mysql, // Usando a data tratada
        ':nf'     => $nota_fiscal,
        ':pp'     => $plano_pgto,
        ':ven'    => $vencimento,
        ':tl'     => $total_liquido,
    ...
    if ($id === '') {
        ...
        // CHECK DE ERRO DO INSERT
        if (!$stmt->execute($params)) {
            $err = $stmt->errorInfo();
            throw new Exception("Erro ao salvar Romaneio (Insert): " . $err[2]);
        }
        
        $romaneioId = $pdo->lastInsertId();
        
        if (empty($romaneioId)) {
            throw new Exception("O banco de dados não retornou um ID válido. Verifique se os dados estão corretos.");
        }
        ...
        // CHECK DE ERRO DO UPDATE
        if (!$stmt->execute($params)) {
            $err = $stmt->errorInfo();
            throw new Exception("Erro ao salvar Romaneio (Update): " . $err[2]);
        }
        $romaneioId = $id;
    }
    ...
    // SE CHEGOU AQUI, TUDO DEU CERTO. CONFIRMA NO BANCO.
    $pdo->commit();
    ...
    // SE DEU ERRO, DESFAZ TUDO (ROLLBACK) E AVISA O USUÁRIO
    if ($pdo->inTransaction()) {
        $pdo->rollBack();
    }
    echo json_encode(['status' => 'erro', 'mensagem' => 'Ocorreu um erro ao salvar: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
    exit;
}