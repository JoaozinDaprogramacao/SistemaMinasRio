...
try {
    ...
    if (!isset($pdo)) {
        throw new Exception("Erro de conexão com o banco de dados. Variável \$pdo não encontrada.");
    }
    ...
    // Atenção: verifique se no HTML o name é 'desc-avista' ou 'desc_avista'. Mantive o seu original 'desc-avista'.
    ...
    // NOVOS campos de configuração (Inputs Opcionais)
    $funrural_config_info       = $_POST['info_funrural'] ?? null;
    $funrural_config_preco_unit = isset($_POST['preco_unit_funrural']) && $_POST['preco_unit_funrural'] !== '' 
                                  ? floatval(str_replace(',', '.', $_POST['preco_unit_funrural'])) : null;

    $ima_config_info            = $_POST['info_ima'] ?? null;
    $ima_config_preco_unit      = isset($_POST['preco_unit_ima']) && $_POST['preco_unit_ima'] !== '' 
                                  ? floatval(str_replace(',', '.', $_POST['preco_unit_ima'])) : null;

    $abanorte_config_info       = $_POST['info_abanorte'] ?? null;
    $abanorte_config_preco_unit = isset($_POST['preco_unit_abanorte']) && $_POST['preco_unit_abanorte'] !== '' 
                                  ? floatval(str_replace(',', '.', $_POST['preco_unit_abanorte'])) : null;

    $taxa_adm_config_taxa_perc  = isset($_POST['taxa_adm_percent']) && $_POST['taxa_adm_percent'] !== '' 
                                  ? floatval(str_replace(',', '.', $_POST['taxa_adm_percent'])) : null;
    $taxa_adm_config_preco_unit = isset($_POST['preco_unit_taxa_adm']) && $_POST['preco_unit_taxa_adm'] !== '' 
                                  ? floatval(str_replace(',', '.', $_POST['preco_unit_taxa_adm'])) : null;


    // --- DESCONTOS DIVERSOS (JSON) ---
    ...
    // Se houver erros, lança exceção para ser capturada pelo catch
    if (!empty($erros)) {
        throw new Exception(implode("<br>", $erros));
    }


    // --- BANCO DE DADOS: PREPARAÇÃO DOS PARÂMETROS ---
    $params = [
        ':forn'  => $fornecedor,
        ':cli'   => $cliente,
        ':qd'    => $quant_dias,
    ...
    if ($id === '') {
        ...
        $sql = "UPDATE {$tabela} SET
                 ...
                 funrural_config_info       = :fci, 
                 funrural_config_preco_unit = :fcpu,
                 ima_config_info            = :ici, 
                 ima_config_preco_unit      = :icpu,
                 abanorte_config_info       = :aci, 
                 abanorte_config_preco_unit = :acpu,
                 taxa_adm_config_taxa_perc  = :atctp, 
                 taxa_adm_config_preco_unit = :atcpu
               WHERE id = :id_val";
    ...
    // --- ITENS DO ROMANEIO (LINHA PRODUTO) ---
    // Limpa itens antigos
    ...
    // --- CAPTURA DE ERROS ---
    // Retorna o erro real (SQL ou PHP) para o Javascript mostrar na tela
    echo json_encode([
        'status'   => 'erro',
