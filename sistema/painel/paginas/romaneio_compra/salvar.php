...
foreach ($valor_1 as $k_prod => $v_prod_str) {
    ...
    if ($v_prod_str !== '' && $v_prod > 0) { // Verifica se o valor original não era vazio também
        $tem_produtos = true;
        if (empty($produto_1[$k_prod]))   { $erros[] = "Selecione variedade em todos os produtos com valor";   break; }
        if (empty($tipo_cx_1[$k_prod]))   { $erros[] = "Selecione tipo de caixa em todos os produtos com valor"; break; }
        if (empty($quant_caixa_1[$k_prod]) || floatval(str_replace(',', '.', $quant_caixa_1[$k_prod])) <= 0) {
            $erros[] = "Quantidade de caixas deve ser maior que zero para produtos com valor"; break;
        }
    }
}
