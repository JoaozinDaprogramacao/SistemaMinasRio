...
<td>
	<big><a class="btn btn-info btn-sm {$inserir_orcamentos}" href="#" onclick="editar('{$id}','{$cliente}','{$tipo}','{$dias_validade}','{$desconto}','{$tipo_desconto}','{$frete}','{$forma_pgto}','{$usuario}','{$obs}')" title="Editar Dados"><i class="fa fa-edit "></i></a></big>

	<div class="icones_mobile" class="dropdown" style="display: inline-block;">                      
                        <a  href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown btn btn-danger btn-sm {$inserir_orcamentos}"><i class="fa fa-trash "></i> </a>
...
<big><a class="{$ocultar} btn btn-success btn-sm" href="#" onclick="baixar('{$id}', '{$nome_cliente}', '{$forma_pgto}', '{$valor}')" title="Aprovar {$tipo}"><i class="fa fa-check-square "></i></a></big>
...
<script type="text/javascript">
	function editar(id, cliente, tipo, dias_validade, desconto, tipo_desconto, frete, forma_pgto, usuario, obs){
		$('#mensagem').text('');
